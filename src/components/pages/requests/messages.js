            ...
            <div className="container mt-3 mb-5" style={{minHeight:'100vh'}}>
                            ...
                            <div className="row mb-2" key={index}>
                                ...
                                <div className="col-12 col-md-8">
                                    {message.read_status === 0 ? 
                                        <Link to={`/message/${message.request_id}/${message.user_id}`} className="mesg-link">
                                            <div className="card pl-1 volunteers-card alert alert-primary" role="alert"> 
                                                <p className="vol-name"><strong>From:</strong> {`${message.user.firstname} ${message.user.lastname}`}</p>
                                                <p className="vol-mesg"><strong>Mesg:</strong> {message.content}</p>
                                                <p className="vol-date"><strong>Date:</strong> {moment(message.created_at).format('LLLL')}</p>
                                            </div>
                                        </Link>
                                    :
                                        <Link to={`/message/${message.request_id}/${message.user_id}`} className="mesg-link">
                ...
                : 
                    <div className="row">
                        <div className="col-md-2"></div>
                        <div className="col-12 col-md-8">
                            <div className="alert alert-primary" role="alert">You've got no messages yet!</div>
                        </div>
                        <div className="col-md-2"></div>
                    </div>
                }
